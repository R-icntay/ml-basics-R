...
numeric_features_long <- estate_data %>% 
  pivot_....(!price_per_unit, names_to = "____", values_to = "____") %>%
  # Group by features
  ....(features) %>% 
  # Calculate correlation coefficient between values and price_per_unit
  mutate(corr_coef = ____) %>% 
  
  # Modifies the feature column to also include corr_coef
  mutate(features = paste(features, ' vs price, r = ', round(corr_coef, 2), sep = '')) %>% 
  ungroup()
...
cat_features_long <- estate_data %>% 
  ____(transaction_date, ____, ____) %>% 
...
estate_train <- ____(estate_split)
estate_test <- ____(estate_split)
...
library(here)
library(rmd2jupyter)
rmd2jupyter("02_Regression_Solution_Ottr_Learner.Rmd")
```
